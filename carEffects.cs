...
public class carEffects : MonoBehaviour
{
    ...
    private void Start() {
        ...
        controller = GetComponent<CarCon>();
    ...
    private void activateSmoke(){
        ...
        if (smokeFlag)
        {
            for (int i = 0; i < smoke.Length; i++)
            {
                ...
                emission.rateOverTime = ((int)controller.KPM * 10 <= 2000) ? (int)controller.KPM * 10 : 2000;
            }
        }
    }
    ...
    public void startSmoke(){
        ...
        for (int i = 0; i < smoke.Length; i++){
            var emission = smoke[i].emission;
            emission.rateOverTime = ((int) controller.KPM *2 >= 2000) ? (int) controller.KPM * 2 : 2000;
            smoke[i].Play();
        }
        smokeFlag = true;
    ...
    private void activateLights() {
        if (IM.vertical < 0 || controller.KPM <= 1) turnLightsOn();
        else turnLightsOff();
    }
